...
* An SSD 1306 OLED display like this one ([https://www.amazon.com/dp/B085WCRS7C?psc=1&ref=ppx_yo2ov_dt_b_product_details](https://www.amazon.com/dp/B085WCRS7C?psc=1&ref=ppx_yo2ov_dt_b_product_details))
...
7. Open a command prompt in your computer and make sure it is connected to the same network as the Raspberry Pi. I used Windows Powershell. Enter the following command and press enter, replacing the hostname and username placeholders with the ones you set when installing the OS to the MicroSD card:
    ...
    (Code Snippet) ssh user@hostname.local

8. Enter your password when prompted. in Windows Powershell there will be no characters shown as you type, so make sure you are typing the correct password, and press enter.
9. Write the following command and press enter:

	(Code Snippet) sudo raspi-config
...
10. Make sure the Operating System is running its latest version. To do so, enter the following command and press enter:

    (Code Snippet) sudo apt-get update && sudo apt-get upgrade -y 

11. Install the necessary drivers, packages and libraries:
    ...
    The following are required ():
    	...
    	(code snippet) sudo apt-get install python3-pip


    	Press enter and enter this command:


    (code snippet) sudo pip3 install --upgrade setuptools


    Press enter and paste the following: \
	(code snippet)
        ...
        wget [https://raw.githubusercontent.com/adafruit/Raspberry-Pi-Installer-Scripts/master/raspi-blinka.py](https://raw.githubusercontent.com/adafruit/Raspberry-Pi-Installer-Scripts/master/raspi-blinka.py)
    ...
    git clone -b v5.8.7 [https://github.com/fastoe/RTL8811CU_for_Raspbian](https://github.com/fastoe/RTL8811CU_for_Raspbian)
...
4. Connect the display to the Raspberry Pi:
    ...
    (code snippet) i2c = busio.I2C(board.SCL, board.SDA) (end snippet)


    Run these two lines:


    (snippet) import adafruit_ssd1306
    ...
    (snippet)oled = adafruit_ssd1306.SSD1306_I2C(128, 64, i2c, addr=0x3d) (end snippet)


    Finally, enter this command:


    (snippet) import digitalio
...
5. The OLED display is now ready to be programmed. To create a python script the display can run, enter the following command to create a file in the text editor and press enter (replace “filename” with the name you want to give to the file):
    ...
    (snippet) nano filename.py (end snippet)
...
7. To start the display’s program without having to use the command line or another computer, enter the following and press enter:
    ...
    (snippet) sudo nano /etc/rc.local (end snippet) 
    ...
    (snippet) python3 /path/filename.py & (end snippet)


    The path may be found by running the command:


    (snippet) find / -name "filename" (end snippet)
...
Setting up the Raspberry Pi
        ...
        1.      Download and run the Raspberry Pi Imager (link goes here)


        2.      Under “Operating System” click “Choose OS”. Scroll down to the “Use Custom” option and click it. Select the .img file that was downloaded in step 1. (Screenshot)


        3.      Place a microSD card into a USB adapter and plug it into the computer. It will be recognized by the computer after a few seconds. (Photo + Screenshot)


        4.      Click the “Choose Storage” button and click it again when the Micro SD shows as a location to install the image. (Screenshot)


        5.      Click “Write” and wait until a completion message shows. (Screenshot)


        6.      Remove the USB adapter and place the MicroSD card into the slot below the Raspberry Pi, like this: (Picture).
...
* Sun May 14 2023 00:11:10 GMT-0700 (PDT)
* Source doc: Instructions Manual Draft
----->
